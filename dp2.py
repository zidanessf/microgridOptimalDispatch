...
electricalLoad = (microgrid_data['交流负荷'] + microgrid_data['直流负荷']).tolist()
...
#inverter
optimalDispatch.inv_ac = Var(T) #inv_ac > 0 means energy flows from inverter to ac side
...
def es_in_out_state(b,i,T,indicator):
    mdl = b.model()
    if indicator == 0:
        b.es_forbidden = Constraint(expr=mdl.es_power_in[i,T]==10)
    else:
        b.es_forbidden = Constraint(expr=mdl.es_power_out[i, T] == 10)
...
'''define constraints'''
'''电功率平衡约束'''
def electricalPowerBalance(mdl,t):
    power_supply = sum(mdl.es_power_out[i,t] for i in N_es) \
                   + sum(mdl.gt_power[i,t] for i in N_gt) \
                   + mdl.utility_power[t]
    power_demand = sum(mdl.es_power_in[i,t] for i in N_es) \
                   + sum(mdl.cs_power[i,t] for i in N_cs) \
                   + sum(mdl.ac_power[i,t] for i in N_ac) \
                   + electricalLoad[t]
    return power_supply == power_demand
optimalDispatch.electricalPowerBalance = Constraint(T,rule=electricalPowerBalance)
'''热功率平衡约束'''
H2M = 0.3
M2L = 0.3
...
df = pd.DataFrame()
df['电负荷'] = pd.Series(electricalLoad)
