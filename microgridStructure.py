...
device_IES = {
    'PV_1' : PV(),
    'Boiler_1' :boiler(Pmax=100),
    'AC_1' : airConditioner(Pmax=80,COP=0.9),
    'GT_1' : gasTurbine(Pmax=500,efficiency = 0.33),
    'GT_2' : gasTurbine(Pmax=500,efficiency = 0.5),
    'ut' : utility(),
    'inv' : inverter(maxP=1000),
    'HS' : heatStorage(capacity=1350,Hmax_in=150,Hmax_out=150)
}
case_IES = MicrogridCase_Simple(device=device_IES, NumOfTime=96)

import pandas as pd
load_PS = pd.read_excel('input_PS.xlsx')
graph_PS = nx.Graph()
